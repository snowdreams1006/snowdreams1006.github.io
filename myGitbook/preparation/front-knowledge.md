...
`markdown` 是一种简化的 `html` 语法,相比于 `txt` 无格式简单文本来说更强大,你可以用专门的软件去编辑 `markdown` 文件,就像使用专门软件编辑 `txt` 文件一样,当然也可以什么软件也不用,甚至直接在记事本或命令行书写,唯一的缺点就是无法实时预览输出效果而已,对了 `markdown` 文件的后缀名是`.md`.
...
`git` 是全世界最先进的分布式版本控制系统,用于帮助项目更好地进行管理,支持版本历史和多人写作管理等功能.

简单地说,是一种高级优雅的文档备份方式,具备云端备份,多人协作等特点.

### 初始化项目

> 语法格式: `git init`

适合从零开始的新项目,初始化完成后才能进行后续的操作.
