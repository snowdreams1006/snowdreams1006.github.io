...
`Go` 内建类型提供了各个长度不等的整型 `int`,其中包括有符号和无符号整型,长度依次是 `1` 个字节,`2` 个字节,`4` 个字节,`8` 个字节长度,分别是 `(u)int` ,`(u)int8` , `(u)int16`, `(u)int32`,`(u)int64` ,除此之外,还有一个指针类型 `uintptr`.

浮点数类型只有 `float` 类型没有 `double` 类型,其中 `float` 的长度依次是 `4` 个字节,`8` 个字节长度,分别是 `float32` ,`float64` .

除了整型和浮点型外,`Go` 还内建支持复数类型,复数的实部和虚部均有 `float` 组成,也就是`float32+float32=complex64`,`float64+float64=complex128` 两种类型.

还有就是字节类型 `byte` 和字符串类型 `string`,意外的是,`Go` 并没有提供字符类型,然而却存在 `rune(int32)` 类型,是 `int32` 类型的别名,但更多被用作字符类型.

当然还有布尔类型 `bool` 表示真假条件.

关于变量类型的相关知识点,你 `Get` 到了吗?欢迎一起学习交流,如需完整源码,请在公众号[雪之梦技术驿站]留言告诉我,感谢你的评论与转发!
