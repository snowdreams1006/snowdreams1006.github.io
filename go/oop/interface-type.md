...
> 如无法自动跳转到公众号「雪之梦技术驿站」文章,可以点击我的头像,动动你的小手翻翻历史文章,相信聪明的你一定可以找到出初识接口的文章.

   接口是面向对象编程风格中继**封装**概念后的另一个重要概念,封装包含两方面含义:**数据和行为**的封装.关于封装的概念这里同样不再赘述,有兴趣的话,可以阅读[go 学习笔记之详细说一说封装是怎么回事](https://mp.weixin.qq.com/s/fXIKWsPqi6m2IEV--8lBsg).
