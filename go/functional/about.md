...
编程世界中从来就没有一家独大的**编程风格**,向来就是百花齐放百家争鸣,除了广为熟悉的**面向对象编程**还有日渐流行的**函数式编程**,至于声明式编程和命令式编程等等其他概念不在此次讨论范围内,因此暂不涉及.

接下来的系列文章中将好好讲一下函数式编程以及 `Go` 语言是如何实现函数式编程的,关于面向对象编程可参考之前的系列文章:
